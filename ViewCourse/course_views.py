...
import ViewCourse.chatbot as chatbot
...
def main(username):
    ...
    selected_query = st.selectbox(
        "query mode", ["AI generate", "Conditional filtering"]
    )
    if selected_query == "AI generate":
        ...
        llm_choice = st.radio(
            "Select the Language Model to Generate SQL", ("Dashscope LLM", "GPT 3.5")
        )
        ...
        subjects_list = (
            subjects_data["subject"].tolist() if not subjects_data.empty else []
        )
        ...
        if not data.empty:
            ...
            selected_columns = st.multiselect(
                "Select columns to display:", all_columns, default=valid_defaults
            )
            ...
            if not filtered_data.empty:
                ...
                if selected_number:
                    ...
                    if submit_button:
                        ...
                        query = f"SELECT course_subject, course_number FROM course_selection WHERE student_id = {username}"
                        filtered_data = db.load_data(query)
                        course_key_set = set()
                        for index, row in filtered_data.iterrows():
                            course_key = (
                                f"({row['course_subject']}, {row['course_number']})"
                            )
                            course_key_set.add(course_key)
                        # the course hasn't been selected
                        if (
                            f"('{selected_subject}', {selected_number})"
                            not in course_key_set
                        ):
                            # Insert sql query
